...
export function AgentToolsTab({ agent, tools, onError }: AgentToolsTabProps) {
  ...
  const getToolInfo = (ref: AgentToolRef) => {
    ...
    const parentExists = !!parentTool;
    const toolExists = !!childTool;
    return {
      parentName: parentTool?.name ?? "Unknown server",
      toolName: ref.toolName,
      description: childTool?.description,
      isAvailable: parentExists && toolExists,
      unavailableReason: !parentExists
        ? "MCP server no longer exists"
        : !toolExists
          ? "Tool no longer available on server"
          : undefined,
    };
  };
      ...
      {agentTools.length === 0 ? (
        ...
        <div className="ui-agent-tools__list">
          {agentTools.map((ref) => {
            ...
            return (
              <div
                key={`${ref.toolId}-${ref.toolName}`}
                className={`ui-agent-tools__item ${!info.isAvailable ? "ui-agent-tools__item--unavailable" : ""}`}
              >
                <div className={`ui-agent-tools__item-icon ${!info.isAvailable ? "ui-agent-tools__item-icon--warning" : ""}`}>
                  {info.isAvailable ? <ToolIcon /> : <WarningIcon />}
                </div>
                <div className="ui-agent-tools__item-content">
                  ...
                  {info.isAvailable && info.description && (
                    <div className="ui-agent-tools__item-desc">{info.description}</div>
                  )}
                  {!info.isAvailable && (
                    <div className="ui-agent-tools__item-warning">{info.unavailableReason}</div>
                  )}
                </div>
                <button
                  type="button"
